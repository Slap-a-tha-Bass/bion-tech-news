          ...
          return (
            ...
            >
              <div className="flex-shrink-0">
                {<img className="h-10" src={getThumbnail(p.data.thumbnail)} alt="" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900">{p.data.title}</p>
                <p className="text-xs text-gray-500 truncate">posted by u/{p.data.author}</p>
              </div>
            </a>
          );
        })}
